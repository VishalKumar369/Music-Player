...
const Register = () => {
		...
		const handleSubmit = (event) => {
			...
			const registration = async () => {
				const response = await axios.post('http://10.0.1.211:3000/user/signup', {
					username: username,
					password: password
				});
		...
		return (
			<div className="bg-[#202020] min-h-screen flex flex-col justify-center items-center px-4">
				<div className="w-full max-w-md p-6 bg-black rounded-2xl shadow-xl">
					...
					<Link to="/login" className="text-white text-center block mt-4 hover:underline">
